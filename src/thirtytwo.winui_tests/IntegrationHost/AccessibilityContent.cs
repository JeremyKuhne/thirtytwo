...
internal sealed class AccessibilityContent : ContentControl
{
    ...
    private void ContentLoaded(object sender, RoutedEventArgs eventArgs)
    {
        ...
        Windows.Application.ColorMode = ApplicationColorMode.Light;
        Ensure(RequestedTheme == ElementTheme.Light, "Light mode did not set the hosted accessibility root theme.");
        EnsureTheme(ElementTheme.Light, "Light");
        _reporter.Write("theme-light-applied");

        Windows.Application.ColorMode = ApplicationColorMode.Dark;
        Ensure(RequestedTheme == ElementTheme.Dark, "Dark mode did not set the hosted accessibility root theme.");
        EnsureTheme(ElementTheme.Dark, "Dark");
        _reporter.Write("theme-dark-applied");

        Windows.Application.ColorMode = ApplicationColorMode.System;
        Ensure(RequestedTheme == ElementTheme.Default, "System mode did not restore the default hosted XAML theme.");
        ...
        Ensure(
            ...
            "System theme was not inherited by every hosted accessibility control.");
        _reporter.Write("theme-system-applied");

        Ensure(_actionButton.Focus(FocusState.Programmatic), "The hosted accessibility action did not accept focus.");
        _reporter.Write("accessibility-ready");
    }

    private void EnsureTheme(ElementTheme expected, string name)
    {
        Ensure(ActualTheme == expected, $"{name} theme did not reach the hosted accessibility root.");
        Ensure(
            ...
            $"{name} theme was not inherited by every hosted accessibility control.");
    }
