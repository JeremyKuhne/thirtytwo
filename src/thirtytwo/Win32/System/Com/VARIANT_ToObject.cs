...
public unsafe partial struct VARIANT : IDisposable
{
    ...
    private static object? ToArray(SAFEARRAY* psa, VARENUM vt)
    {
        ...
        HRESULT hr = Interop.SafeArrayLock(psa);
        ...
        {
            hr = Interop.SafeArrayUnlock(psa);
            Debug.Assert(hr == HRESULT.S_OK);
        }
    ...
    private static Array CreateArrayFromSafeArray(SAFEARRAY* psa, VARENUM vt)
    {
        ...
        if (vt == VT_RECORD)
        {
            ...
            Interop.SafeArrayGetRecordInfo(psa, record).ThrowOnFailure();
    ...
    private static Span<T> GetSpan<T>(Array arr)
        => MemoryMarshal.CreateSpan(ref Unsafe.AsRef<T>(InteropMarshal.UnsafeAddrOfPinnedArrayElement(arr, 0).ToPointer()), arr.Length);
