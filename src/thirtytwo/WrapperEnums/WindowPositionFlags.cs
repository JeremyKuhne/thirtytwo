...
/// <summary>
///  Controls which position, size, activation, and redraw changes <c>SetWindowPos</c> applies.
/// </summary>
[Flags]
public enum WindowPositionFlags : uint
{
    /// <summary>
    ///  Applies the requested bounds, z-order, activation, and redraw changes.
    /// </summary>
    None = 0,

    /// <summary>
    ///  Retains the current size and ignores the requested width and height.
    /// </summary>
    NoSize = SET_WINDOW_POS_FLAGS.SWP_NOSIZE,

    /// <summary>
    ///  Retains the current position and ignores the requested X and Y coordinates.
    /// </summary>
    NoMove = SET_WINDOW_POS_FLAGS.SWP_NOMOVE,

    /// <summary>
    ///  Retains the current z-order and ignores the requested insertion position.
    /// </summary>
    NoZOrder = SET_WINDOW_POS_FLAGS.SWP_NOZORDER,

    /// <summary>
    ///  Suppresses repainting caused by the position change.
    /// </summary>
    ...
    /// <summary>
    ///  Recalculates the nonclient area even when the window size does not change.
    /// </summary>
    ...
    /// <summary>
    ///  Discards the current client-area contents instead of preserving them after the change.
    /// </summary>
    ...
    /// <summary>
    ///  Prevents the window from receiving <c>WM_WINDOWPOSCHANGING</c>.
    /// </summary>
    NoSendChanging = SET_WINDOW_POS_FLAGS.SWP_NOSENDCHANGING,

    /// <summary>
    ///  Prevents generation of <c>WM_SYNCPAINT</c>.
    /// </summary>
